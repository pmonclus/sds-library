...
2. A constructor function auto-registers the array before `main()` runs
3. `sds_register_table()` looks up the table type and uses the metadata
4. No runtime `switch(type)` - just direct function pointer calls
